...
char *create_name (char *name) {
  char *copied_name = (char *)malloc(sizeof(char)*(strlen(name)+1));
...
struct name_info *create_name_info (char *name) {
  struct name_info *result = (struct name_info *)malloc(sizeof(struct name_info));
  result->name = create_name(name);
  result->total_tracking = 0;
  return result;
	/* YOUR CODE HERE */
}
...
struct user_info *create_user (char *name) {
	/* YOUR CODE HERE. */
  struct user_info *result = (struct user_info *)malloc(sizeof(struct user_info));
  result->name_info = create_name_info(name);
  result->nickname = (char **)malloc(sizeof(char *));
  *(result->nickname) = create_name(name);
  result->muted_capacity = 10;
  result->muted = (struct name_info **)malloc(result->muted_capacity * sizeof(struct name_info *));
  result->muted_total = 0;
  for (int i = 0; i < result->muted_capacity; i ++ ) {
    result->muted[i] = NULL;
  }
  return result;
}
...
void cleanup_user (struct user_info *user) {
  // struct user_info *to_be_cleaned_up = find_user(user->name_info->name);
  // if (to_be_cleaned_up == NULL) {
...
void cleanup_name_info (struct name_info *info) {
	/* YOUR CODE HERE. */
  // struct user_info *to_be_cleaned_up = find_user(info->name);
  // if (to_be_cleaned_up == NULL) {
  free(info->name);
  free(info);
  // }
...
bool has_nickname (struct user_info *user) {
	/* YOUR CODE HERE. */
  if (strcmp(user->name_info->name, *(user->nickname)) == 0) {
    return 0;
  }
  return 1;
}
...
bool istaken_nickname (char *name) {
  ...
  while (count < socket_total) {
    if (sockets[i] != -1) {
      ...
      if ((user = users[i]) != NULL && has_nickname(user) && strcmp(*(user->nickname), name) == 0) {
        return 1;
      }
      count++;
    }
...
bool ismuted (struct user_info *receiving_user, struct user_info *possibly_muted_user) {
  for (int i = 0; i < receiving_user->muted_total; i++) {
    if(receiving_user->muted[i] == possibly_muted_user->name_info) {
      return true;
    }
  }
  return false;
