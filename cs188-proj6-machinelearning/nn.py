...
class Graph(object):
    ...
    def add(self, node):
        ...
        self.inputs[node] = [self.get_output(parent) for parent in parents]
        self.output[node] = node.forward(self.inputs[node])
        self.accumulator[node] = np.zeros_like(self.output[node])
    ...
    def backprop(self):
        ...
        self.accumulator[last_node] = np.ones_like(self.accumulator[last_node])
        for node in reversed(self.nodes):
            parents = node.get_parents()
            length = len(parents);
            rec = node.backward(self.get_inputs(node), self.get_gradient(node))
            for i in range(length):
                parent = parents[i]
                self.accumulator[parent] += rec[i]
    ...
    def step(self, step_size):
        ...
        for node in self.variables:
            node.data = node.data - self.accumulator[node] * step_size
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        A = inputs[0]
        B = inputs[1]
        return [np.dot(gradient, np.transpose(B)), np.dot(np.transpose(A), gradient)]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient, np.dot(np.transpose(gradient), np.ones(len(inputs[0])))]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.where(inputs[0] < 0, 0, 1) * gradient]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.mean(np.dot(0.5, np.square(inputs[0]-inputs[1])))
    ...
    def backward(inputs, gradient):
        ...
        B = inputs[1]
        mat = (A-B)
        return [mat*gradient/len(mat), -mat*gradient/len(mat)]
