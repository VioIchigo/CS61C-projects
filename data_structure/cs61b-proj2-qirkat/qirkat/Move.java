...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            return move(move0._col0, move0._row0,
                    move1._col0, move1._row0, move1._nextJump);
        }
        if (move0.jumpTail() == null) {
            ...
            return move(move0._col0, move0._row0,
                    move0._col1, move0._row1, newNext);
        }
    ...
    boolean isJump() {
        boolean partial = (abs(_col1 - _col0) == 0 && abs(_row1 - _row0) == 0)
                || (abs(_col1 - _col0) == 0 && abs(_row1 - _row0) == 2)
                || (abs(_col1 - _col0) == 2 && abs(_row1 - _row0) == 2)
                || (abs(_col1 - _col0) == 2 && abs(_row1 - _row0) == 0);
        return _isJump && partial;
    }
    ...
    boolean isLeftMove() {
        return _row0 == _row1
                && index(_col0, _row0) - index(_col1, _row1) == 1;
    }
    ...
    char jumpedRow() {
        if (isJump()) {
            return row((_fromIndex + _toIndex) / 2);
        } else {
            return _row1;
        }
    }
    ...
    char jumpedCol() {
        if (isJump()) {
            return col((_fromIndex + _toIndex) / 2);
        } else {
            return _col1;
        }
    }
    ...
    private void toString(Formatter out) {
        out.format("%s%s", _col0, _row0);
        toStringHelper(this, out);
        Move iter = _nextJump;
        while (iter != null) {
            toStringHelper(iter, out);
            iter = iter._nextJump;
        }
    }

    /** A helper function that takes in a formatter OUT
     *  Move MOVE and changes out. */
    private void toStringHelper(Move move, Formatter out) {
        out.format("-");
        out.format("%s%s", move._col1, move._row1);
    }
