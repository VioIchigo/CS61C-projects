...
void handle_set_nickname (char **args, unsigned count, unsigned n) {
 ...
 /* REPLACE ME */
  if (*(user->nickname) != args[1]) {
    free(*(user->nickname));
    *(user->nickname) = create_name(args[1]);
  } else {
    handle_invalid_arguments("set_nickname", n);
    return;
  }
...
void handle_rename (char **args, unsigned count, unsigned n) {
  ...
  if (count != 1 || !isvalidname(args[0])) {
	...
	char *old_name = (char *)malloc(sizeof(char)*(strlen(user->name_info->name)+1));
    strcpy(old_name, user->name_info->name);
 ...
 * by the user who called the command, set that user to be muted by the
...
void handle_mute (char **args, unsigned count, unsigned n) {
  ...
  if (count != 1 || !istaken_name(args[0])) {
    handle_invalid_arguments("mute", n);
    return;
  }
	/* IMPLEMENT THE CORE FUNCTIONALITY */
  struct user_info *to_be_muted = find_user(args[0]);
	struct user_info *user = users[n];
  if (strcmp(to_be_muted->name_info->name, user->name_info->name) == 0) {
    handle_invalid_arguments("mute", n);
    return;
  }

  if (!ismuted(user, to_be_muted)) {
    user->muted_total = user->muted_total + 1;
    user->muted[user->muted_total - 1] = to_be_muted->name_info;
    to_be_muted->name_info->total_tracking += 1;
  }
...
void handle_unmute (char **args, unsigned count, unsigned n) {
  ...
  if (count != 1 || !istaken_name(args[0])) {
	...
	/* IMPLEMENT THE CORE FUNCTIONALITY */
  struct user_info *to_be_unmuted = find_user(args[0]);
	struct user_info *user = users[n];
  if (strcmp(to_be_unmuted->name_info->name, user->name_info->name) == 0) {
    handle_invalid_arguments("unmute", n);
    return;
  }

  if (ismuted(user, to_be_unmuted)) {
    int index;
    for (index = 0; index < user->muted_total; index++) {
      if (user->muted[index] == to_be_unmuted->name_info) {
        break;
      }
    }

    user->muted[index]->total_tracking -= 1;

    if (index != user->muted_total - 1) {
      user->muted[index] = user->muted[user->muted_total - 1];
    }
...
void handle_show_status (char **args, unsigned count, unsigned n) {
        ...
        struct user_info *to_be_showed = find_user(args[0]);
        output_user_status (to_be_showed, n);
