...
int find_message_end (char *msg, int start) {
        /* YOUR CODE HERE. */
        for (int i = start; i < MAX_MESSAGE_LENGTH; i++) {
          if (*(msg+i) == '\0') {
            break;
          }
          if (*(msg+i) == '\n') {
 ...
 * buffer contains 1 or messages wherse a message is defined as being a
...
char *generate_message (char *messages, int end) {
       /* YOUR CODE HERE. */
       char *first_message = (char  *)malloc(sizeof(char)*(end + 1));
       strncpy(first_message, messages, end);
       for (int i = end; i < MAX_MESSAGE_LENGTH; i++) {
